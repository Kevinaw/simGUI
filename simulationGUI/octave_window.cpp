...
OctaveWindow::OctaveWindow(QDockWidget *parent_): QWidget()
{
  ...
  font.setPointSize(SimguiSettings.font.pointSize()-1);
  ...
  output->setPaletteBackgroundColor(SimguiSettings.BGColor);
...
bool OctaveWindow::startOctave()
{
  ...
  QString OctavePath=SimguiSettings.OctaveBinDir.canonicalPath();
  ...
  Arguments << "--no-history" << "-i" << "-f" << "-p"
            << QDir::toNativeSeparators(SimguiSettings.OctaveDir); // m-files location
...
{
  sendCommand("cd \"" + SimguiSettings.SimguiWorkDir.absPath() + "\"");
}
