...
{
  setWindowTitle(tr("Simgui Simulation Messages"));
  SimguiDoc *Doc;
  DocWidget = w;
  if(DocWidget->inherits("QTextEdit"))
    Doc = (SimguiDoc*) ((TextDoc*)DocWidget);
  else
    Doc = (SimguiDoc*) ((Schematic*)DocWidget);
...
bool SimMessage::startProcess()
{
 ...
 /* On Qt4 it shows as running even before we .start it. FIXME*/
  ...
  NetlistFile.setName(SimguiSettings.SimguiHomeDir.filePath("netlist.txt"));
...
// ---------------------------------------------------
// Converts a spice netlist into Simgui format and outputs it.
...
// FIXME #warning SPICE section below not being covered?
  ...
  QString prog;
  QStringList com;
  prog = SimguiSettings.BinDir + "simguiconv" + executableSuffix;
  if(makeSubcircuit)
    com << "-g" << "_ref";
  com << "-if" << "spice" << "-of" << "simgui";

  QFile SpiceFile;
  if(FileName.find(QDir::separator()) < 0)  // add path ?
    SpiceFile.setName(SimguiSettings.SimguiWorkDir.path() + QDir::separator() + FileName);
  else
  ...
  qDebug() << "start SimguiConv" << prog << com.join(" ");
  SimProcess.start(prog, com);

  if(!SimProcess.Running) {
    ErrText->insert(tr("SIM ERROR: Cannot start SimguiConv!"));
    FinishSimulation(-1);
    return;
  }
...
//  QString SimPath = QDir::convertSeparators (SimguiSettings.SimguiHomeDir.absPath());
//#ifdef __MINGW32__
//  QString SimguiDigiLib = "simguidigilib.bat";
//  QString SimguiDigi = "simguidigi.bat";
//  QString SimguiVeri = "simguiveri.bat";
//#else
//  QString SimguiDigiLib = "simguidigilib";
//  QString SimguiDigi = "simguidigi";
//  QString SimguiVeri = "simguiveri";
//#endif
//  SimOpt = NULL;
...
//      }
//#endif
//      Program = pathName(SimguiSettings.BinDir + SimguiDigi);
//      Arguments  << SimguiSettings.SimguiHomeDir.filePath("netlist.txt")
//                 << DataSet << SimTime << pathName(SimPath)
//                 << pathName(SimguiSettings.BinDir) << libs;
//    }
//    // Module.
//    else {
...
//      QString dir = QDir::convertSeparators (SimguiSettings.SimguiHomeDir.path());
//      QDir vhdlDir(dir);
...
//      destFile.close();
//      Program = pathName(SimguiSettings.BinDir + SimguiDigiLib);
//      Arguments << SimguiSettings.SimguiHomeDir.filePath("netlist.txt")
//                << pathName(SimPath)
//                << entity
//                << lib;
...
//
//          /*! Only pass modules to Simguiator that are indeed used on
//           * the schematic,it might be the case that the user loaded the icons,
//           * but did not compiled the module. Simguiator will not find the library.
//           *
//           * Check if used symbols have corresponing lib before running
//           * Simguiator? Need to search on the netlis.txt? Is there other data
...
//            //Arguments << "-p" << SimguiSettings.SimguiWorkDir.absolutePath()
//            //          << "-m" << usedComponents;
...
//               out << "\n";
//               out << "# --path=" << SimguiSettings.SimguiWorkDir.absolutePath() << "\n";
...
//
//        Program = SimguiSettings.AscoBinDir.canonicalPath();
//        Program = QDir::toNativeSeparators(Program+"/"+"asco"+QString(executableSuffix));
//        Arguments << "-simgui" << SimguiSettings.SimguiHomeDir.filePath("asco_netlist.txt")
//                  << "-o" << "asco_out";
//      }
//      else {
//        Program = SimguiSettings.BinDir + "simguiator" + executableSuffix;
//        Arguments << "-b" << "-g" << "-i"
//                  << SimguiSettings.SimguiHomeDir.filePath("netlist.txt")
//                  << "-o" << DataSet;
//      }
//    }
//    else {
//      if (isVerilog) {
//          Program = QDir::toNativeSeparators(SimguiSettings.BinDir + SimguiVeri);
//          Arguments << QDir::toNativeSeparators(SimguiSettings.SimguiHomeDir.filePath("netlist.txt"))
//                    << DataSet
//                    << SimTime
//                    << QDir::toNativeSeparators(SimPath)
//                    << QDir::toNativeSeparators(SimguiSettings.BinDir)
//                    << "-c";
//      } else {
///// \todo \bug error: unrecognized command line option '-Wl'
//#ifdef __MINGW32__
//    Program = QDir::toNativeSeparators(pathName(SimguiSettings.BinDir + SimguiDigi));
//    Arguments << QDir::toNativeSeparators(SimguiSettings.SimguiHomeDir.filePath("netlist.txt"))
//              << DataSet
//              << SimTime
//              << QDir::toNativeSeparators(SimPath)
//              << QDir::toNativeSeparators(SimguiSettings.BinDir) << "-Wall" << "-c";
//#else
//    Program = QDir::toNativeSeparators(pathName(SimguiSettings.BinDir + SimguiDigi));
//    Arguments << SimguiSettings.SimguiHomeDir.filePath("netlist.txt")
//              << DataSet << SimTime << pathName(SimPath)
//		      << pathName(SimguiSettings.BinDir) << "-Wall" << "-c";
//
//#endif
//      }
//    }
...
//  // insert Simgui bin dir, so ASCO can find simguiator
//  env.insert("PATH", env.value("PATH") + sep + SimguiSettings.BinDir );
...
void SimMessage::FinishSimulation(int Status)
{
  ...
  QFile file(SimguiSettings.SimguiHomeDir.filePath("log.txt"));  // save simulator messages
  ...
  if(Status == 0) {
    if(SimOpt) { // save optimization data
      QFile ifile(SimguiSettings.SimguiHomeDir.filePath("asco_out.dat"));
      QFile ofile(DataSet);
