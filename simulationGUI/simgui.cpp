...
SimguiApp::SimguiApp()
{
  setCaption("Circuit Simulation " PACKAGE_VERSION);
  ...
  SimguiFileFilter =
    ...
    tr("Any File")+" (*)";
  //SimguiSettings.SimguiWorkDir.setPath(QDir::homeDirPath()+QDir::convertSeparators ("/.simgui"));
  //SimguiSettings.SimguiHomeDir.setPath(QDir::homeDirPath()+QDir::convertSeparators ("/.simgui"));

  updateSchNameHash();
  updateSpiceNameHash();

  move  (SimguiSettings.x,  SimguiSettings.y);
  resize(SimguiSettings.dx, SimguiSettings.dy);
  ...
  for(int z=1; z<qApp->argc(); z++) {
    ...
    if(*(arg) != '-') {
      ...
      //  gotoPage(f);
      //} else {
        // get and set absolute path, SimguiWorkDir now finds subcircuits
        QFileInfo Info(arg);
        SimguiSettings.SimguiWorkDir.setPath(Info.absoluteDir().absolutePath());
        arg = SimguiSettings.SimguiWorkDir.filePath(Info.fileName());
        gotoPage(arg);
      //}
    }
  }
...
// ##########                                                   ##########
// #######################################################################
void SimguiApp::initContentListView()
{
...
/**
 * @brief SimguiApp::initView Setup the layour of all widgets
 */
void SimguiApp::initView()
{
...
//#ifndef __APPLE__
//  setWindowIcon (QPixmap(":/bitmaps/big.simgui.xpm"));
//#endif
...
void SimguiApp::fillLibrariesTreeView ()
{
...
//    newitem->setBackground
    ...
    QDir LibDir(SimguiSettings.LibDir);
    ...
    for(it = LibFiles.begin(); it != LibFiles.end(); it++)
    {
        ...
        for (int i = 0; i < parsedlibrary.components.count (); i++)
        {
            ...
            QString s = "<Simgui Schematic " PACKAGE_VERSION ">\n";
    ...
    QDir UserLibDir = QDir (SimguiSettings.SimguiHomeDir.canonicalPath () + "/user_lib/");
...
// ---------------------------------------------------------------
// Returns a pointer to the SimguiDoc object whose number is "No".
// If No < 0 then a pointer to the current document is returned.
SimguiDoc* SimguiApp::getDoc(int No)
{
  QWidget *w;
  if(No < 0)
  ...
  if(w) {
    if(isTextDocument (w))
      return (SimguiDoc*) ((TextDoc*)w);
    else
      return (SimguiDoc*) ((Schematic*)w);
  }
...
// ---------------------------------------------------------------
// Returns a pointer to the SimguiDoc object whose file name is "Name".
SimguiDoc * SimguiApp::findDoc (QString File, int * Pos)
{
  SimguiDoc * d;
  int No = 0;
...
void SimguiApp::fillComboBox (bool setAll)
{
...
void SimguiApp::slotSetCompView (int index)
{
//  qDebug() << "SimguiApp::slotSetCompView (int index)";
  ...
  if (item == QObject::tr("verilog-a user devices")) {
    ...
    while (i.hasNext()) {
      ...
      QString iconPath = SimguiSettings.SimguiWorkDir.filePath(vaBitmap+".png");
...
void SimguiApp::slotSelectComponent(QListWidgetItem *item)
{
  editText->setHidden(true); // disable text edit of component property
...
// ----------------------------------------------------------
// Shows the menu.
void SimguiApp::slotShowContentMenu(const QPoint& pos) {
...
/* OLD Version
void SimguiApp::slotShowContentMenu(Q3ListViewItem *item, const QPoint& point, int)
{
  if(item)
...
// ----------------------------------------------------------
void SimguiApp::slotCMenuCopy()
{
  QTreeWidgetItem *Item = Content->currentItem();
  if(Item == 0) return;

  QString Name = Item->text(0);
  QString currentPath = SimguiSettings.SimguiWorkDir.filePath(Name);
  QString Path = currentPath.section(QDir::separator(), 0, -2);

  //check changed file save
  int z = 0; //search if the doc is loaded
  SimguiDoc *d = findDoc(currentPath, &z);
  if (d != NULL && d->DocChanged) {
    DocumentTab->setCurrentPage(z);
    int ret = QMessageBox::question(this, tr("Copying Simgui document"), 
...
// ----------------------------------------------------------
void SimguiApp::slotCMenuRename()
{
  QTreeWidgetItem *Item = Content->currentItem();
  if(!Item) return;

  QString Name = Item->text(0);
  if (findDoc (SimguiSettings.SimguiWorkDir.filePath(Name))) {
  ...
  if(s.contains('.'))
    ...
    NewName = s+"."+Suffix;
  QDir file(SimguiSettings.SimguiWorkDir.path());
...
// ----------------------------------------------------------
void SimguiApp::slotCMenuDelete()
{
  QTreeWidgetItem *item = Content->currentItem();
  if(item == 0) return;
  QString FileName = SimguiSettings.SimguiWorkDir.filePath(item->text(0));
...
// ----------------------------------------------------------
QString SimguiApp::fileType (const QString& Ext)
{
...
void SimguiApp::slotCMenuDelGroup ()
{
  QTreeWidgetItem *item = Content->currentItem();
  if (item == 0)
    return;
  QString s = item->text (0);
  s = SimguiDoc::fileBase (s); // cut off suffix from file name
  ...
  for (i = 0; extensions[i] != 0; i++) {
    ...
    QString Name = SimguiSettings.SimguiWorkDir.filePath (Short);
  ...
  for (i = 0; extensions[i] != 0; i++) {
    QString Short = s + "." + extensions[i];
    QString Long = SimguiSettings.SimguiWorkDir.filePath (Short);
...
void SimguiApp::slotCMenuInsert ()
{
...
// Checks for simgui directory and reads all existing Simgui projects.
void SimguiApp::readProjects()
{
  QDir ProjDir(SimguiSettings.SimguiHomeDir);
  if(!ProjDir.cd(SimguiSettings.SimguiHomeDir.absolutePath())) { // work directory exists ?
    if(!ProjDir.mkdir(SimguiSettings.SimguiHomeDir.absolutePath())) { // no, then create it
      QMessageBox::warning(this, tr("Warning"),
                   tr("Cannot create work directory !"));
      return;
    }
    ProjDir.cd(SimguiSettings.SimguiHomeDir.absolutePath());
  }
...
void SimguiApp::slotProjNewButt()
{
  editText->setHidden(true); // disable text edit of component property
  ...
  QDir projDir(SimguiSettings.SimguiHomeDir.path());
...
// ----------------------------------------------------------
// Checks whether this file is a simgui file and whether it is an subcircuit.
// It returns the number of subcircuit ports.
int SimguiApp::testFile(const QString& DocName)
{
  QFile file(DocName);
  ...
  if(Line.left(16) != "<Simgui Schematic ") {  // wrong file type ?
    file.close();
    return -3;
  }
  ...
  if(!checkVersion(Line)) { // wrong version number ?
      if (!SimguiSettings.IgnoreFutureVersion) {
          file.close();
          return -4;
      }
    //file.close();
...
// content ListView
void SimguiApp::readProjectFiles()
{
...
*/
  ...
  QStringList Elements = SimguiSettings.SimguiWorkDir.entryList("*", QDir::Files, QDir::Name);
  QStringList::iterator it;
  QString Str;
  ConSchematics->setExpanded(true);
  for(it = Elements.begin(); it != Elements.end(); ++it) {
    Str = SimguiDoc::fileSuffix (*it);
    if(Str == "sch") {
      n = testFile(SimguiSettings.SimguiWorkDir.filePath((*it).ascii()));
      if(n >= 0) {
        if(n > 0) {
...
void SimguiApp::openProject(const QString& Path, const QString& Name)
{
  editText->setHidden(true); // disable text edit of component property
  ...
  SimguiSettings.SimguiWorkDir.setPath(ProjDir.path());
...
void SimguiApp::slotProjOpenButt()
{
  editText->setHidden(true); // disable text edit of component property
...
void SimguiApp::slotOpenProject(QListWidgetItem *item)
{
  openProject(SimguiSettings.SimguiHomeDir.filePath(item->text()+"_prj"), item->text());
}

// ----------------------------------------------------------
// Is called when the close project menu is called.
void SimguiApp::slotMenuCloseProject()
{
  editText->setHidden(true); // disable text edit of component property
  ...
  slotResetWarnings();
  setCaption("Simgui " PACKAGE_VERSION + tr(" - Project: "));
  SimguiSettings.SimguiWorkDir.setPath(QDir::homeDirPath()+QDir::convertSeparators ("/.simgui"));
  octave->adjustDirectory();
...
// ----------------------------------------------------------
bool SimguiApp::deleteDirectoryContent(QDir& Dir)
{
...
// ----------------------------------------------------------
bool SimguiApp::deleteProject(const QString& Path, const QString& Name)
{
  editText->setHidden(true); // disable text edit of component property
...
void SimguiApp::slotMenuDelProject()
{
...
void SimguiApp::slotProjDelButt()
{
  QListWidgetItem *item = Projects->currentItem();
  if(!item) {
    QMessageBox::information(this, tr("Info"),
  ...
  if(!deleteProject(SimguiSettings.SimguiHomeDir.filePath(item->text()+"_prj"),
	item->text()))  return;
...
bool SimguiApp::gotoPage(const QString& Name)
{
  int No = DocumentTab->currentPageIndex();

  int i = 0;
  SimguiDoc * d = findDoc (Name, &i);  // search, if page is already loaded
...
void SimguiApp::slotFileOpen()
{
  ...
  QString s = QFileDialog::getOpenFileName(this, tr("Enter a Schematic Name"),
    lastDirOpenSave.isEmpty() ? QString(".") : lastDirOpenSave, SimguiFileFilter);
...
// --------------------------------------------------------------
bool SimguiApp::saveFile(SimguiDoc *Doc)
{
  if(!Doc)
    Doc = getDoc();
...
bool SimguiApp::saveAs()
{
  ...
  while(true) {
    ...
    if(s.isEmpty()) {   // which is default directory ?
      ...
      else s = SimguiSettings.SimguiWorkDir.path();
    }
    ...
    if(isTextDocument (w))
      Filter = tr("VHDL Sources")+" (*.vhdl *.vhd);;" +
	       ...
	       tr("Verilog-A Sources")+" (*.va);;"+
	       tr("Octave Scripts")+" (*.m *.oct);;"+
	       tr("Simgui Netlist")+" (*.net *.qnet);;"+
	       tr("Plain Text")+" (*.txt);;"+
	       tr("Any File")+" (*)";
    else
      Filter = SimguiFileFilter;

    s = QFileDialog::getSaveFileName(this, tr("Enter a Document Name"),
                                     SimguiSettings.SimguiWorkDir.absPath(),
                                     Filter);
  ...
  if(intoView) {    // insert new name in Content ListView ?
    if(Info.dirPath(true) == SimguiSettings.SimguiWorkDir.absPath())
      if(!ProjName.isEmpty()) {
...
// --------------------------------------------------------------
void SimguiApp::slotFileSaveAll()
{
  ...
  int No=0;
  SimguiDoc *Doc;  // search, if page is already loaded
...
void SimguiApp::slotFileClose(int index)
{
...
void SimguiApp::closeFile(int index)
{
    statusBar()->message(tr("Closing file..."));
    editText->setHidden(true); // disable text edit of component property

    SimguiDoc *Doc = getDoc(index);
    if(Doc->DocChanged) {
      switch(QMessageBox::warning(this,tr("Closing Simgui document"),
        tr("The document contains unsaved changes!\n")+
...
void SimguiApp::slotFileExamples()
{
  statusBar()->message(tr("Open examples directory..."));
  QString path = QDir::toNativeSeparators(SimguiSettings.ExamplesDir);
  QDesktopServices::openUrl(QUrl("file:///" + path.replace("\\","/")));
  statusBar()->message(tr("Ready."));
}

void SimguiApp::slotHelpTutorial()
{
  QString path = QDir::toNativeSeparators(SimguiSettings.DocDir);
  QUrl url = QUrl("file:///" + path.replace("\\","/") + "tutorial/" + QObject::sender()->objectName());
  QDesktopServices::openUrl(url);
}

void SimguiApp::slotHelpTechnical()
{
  QString path = QDir::toNativeSeparators(SimguiSettings.DocDir);
...
void SimguiApp::slotChangeView(QWidget *w)
{
...
// --------------------------------------------------------------
void SimguiApp::slotFileSettings ()
{
  editText->setHidden (true); // disable text edit of component property

  QWidget * w = DocumentTab->currentPage ();
  if (isTextDocument (w)) {
    SimguiDoc * Doc = (SimguiDoc *) ((TextDoc *) w);
...
// --------------------------------------------------------------
void SimguiApp::slotApplSettings()
{
  editText->setHidden(true); // disable text edit of component property

  SimguiSettingsDialog *d = new SimguiSettingsDialog(this);
  d->exec();
  view->drawn = false;
}
...
// --------------------------------------------------------------
void SimguiApp::slotRefreshSchPath()
{
  this->updateSchNameHash();
  this->updateSpiceNameHash();
...
// --------------------------------------------------------------
void SimguiApp::updatePortNumber(SimguiDoc *currDoc, int No)
{
  if(No<0) return;
...
void SimguiApp::printCurrentDocument(bool fitToPage)
{
...
void SimguiApp::slotFilePrintFit()
{
...
void SimguiApp::closeEvent(QCloseEvent* Event)
{
    qDebug()<<"x"<<pos().x()<<" ,y"<<pos().y();
    qDebug()<<"dx"<<size().width()<<" ,dy"<<size().height();
    SimguiSettings.x=pos().x();
    SimguiSettings.y=pos().y();
    SimguiSettings.dx=size().width();
    SimguiSettings.dy=size().height();
...
void SimguiApp::slotIntoHierarchy()
{
  editText->setHidden(true); // disable text edit of component property
...
void SimguiApp::slotPopHierarchy()
{
  editText->setHidden(true); // disable text edit of component property
...
/*!
 * \brief SimguiApp::slotSimulate
 *  is called when the simulate toolbar button is pressed.
 */
void SimguiApp::slotSimulate()
{
  editText->setHidden(true); // disable text edit of component property
...
void SimguiApp::slotAfterSimulation(int Status, SimMessage *sim)
{
...
void SimguiApp::slotChangePage(QString& DocName, QString& DataDisplay)
{
  ...
  SimguiDoc * d = findDoc (Name, &z);

  if(d)
    DocumentTab->setCurrentPage(z);
  else {   // no open page found ?
    QString ext = SimguiDoc::fileSuffix (DataDisplay);
...
void SimguiApp::slotOpenContent(QTreeWidgetItem *item)
{
  editText->setHidden(true); // disable text edit of component property
...
/*
  SimguiSettings.SimguiWorkDir.setPath(SimguiSettings.SimguiHomeDir.path());
  QString p = ProjName+"_prj";
  if(!SimguiSettings.SimguiWorkDir.cd(p)) {
    QMessageBox::critical(this, tr("Error"),
			  tr("Cannot access project directory: ")+
              SimguiSettings.SimguiWorkDir.path()+QDir::separator()+p);
    return;
  }*/

  QFileInfo Info(SimguiSettings.SimguiWorkDir.filePath(item->text(0)));
  ...
  // File is no Simgui file, so go through list and search a user
...
void SimguiApp::slotSelectSubcircuit(QTreeWidgetItem *item)
{
  editText->setHidden(true); // disable text edit of component property
...
// ---------------------------------------------------------
// Is called when the mouse is clicked within the Content QListView.
void SimguiApp::slotSelectLibComponent(QTreeWidgetItem *item)
{
...
void SimguiApp::switchSchematicDoc (bool SchematicMode)
{
...
// ---------------------------------------------------------
void SimguiApp::switchEditMode(bool SchematicMode)
{
...
// ---------------------------------------------------------
void SimguiApp::changeSchematicSymbolMode(Schematic *Doc)
{
  if(Doc->symbolMode) {
...
// ---------------------------------------------------------
bool SimguiApp::isTextDocument(QWidget *w) {
...
void SimguiApp::slotSymbolEdit()
{
  ...
  if (isTextDocument (w)) {
    ...
    int paint_mode = 0;
    if (!findDoc (SimguiSettings.SimguiWorkDir.filePath(sym)))
      paint_mode = 1;
...
// -----------------------------------------------------------
void SimguiApp::slotPowerMatching()
{
...
// -----------------------------------------------------------
void SimguiApp::slot2PortMatching()
{
  if(!view->focusElement) return;
  if(view->focusElement->Type != isMarker) return;
  Marker *pm = (Marker*)view->focusElement;
...
void SimguiApp::slotEditElement()
{
  if(view->focusMEvent)
...
// looses the focus.
void SimguiApp::slotHideEdit()
{
  editText->setHidden(true);
}

// -----------------------------------------------------------
// Searches the simgui path list for all schematic files and creates
// a hash for lookup later
void SimguiApp::updateSchNameHash(void)
{
    // update the list of paths to search in simguiPathList, this
    ...
    foreach (QString simguipath, simguiPathList) {
        QDir thispath(simguipath);
    ...
    QDir thispath(SimguiSettings.SimguiWorkDir);
    QFileInfoList schfilesList = thispath.entryInfoList( nameFilter, QDir::Files );
    // put each one in the hash table with the unique key the base name of
    // the file, note this will overwrite the value if the key already exists
    foreach (QFileInfo schfile, schfilesList) {
...
QString SimguiApp::getSpiceFileFilter (void)
{
    QString spexts = " (";
...
// -----------------------------------------------------------
// Searches the simgui path list for all spice files and creates
// a hash for lookup later
void SimguiApp::updateSpiceNameHash(void)
{
    // update the list of paths to search in simguiPathList, this
    // removes nonexisting entries
    updatePathList();
...
void SimguiApp::updatePathList(void)
{
    // check each path actually exists, if not remove it
    QMutableListIterator<QString> i(simguiPathList);
    while (i.hasNext()) {
        i.next();
...
void SimguiApp::updatePathList(QStringList newPathList)
{
    // clear out the old path list
    simguiPathList.clear();

    // copy the new path into the path list
    foreach(QString path, newPathList)
    {
        simguiPathList.append(path);
    }
...
void SimguiApp::updateRecentFilesList(QString s)
{
    QSettings* settings = new QSettings("simgui","simgui");
    SimguiSettings.numRecentDocs++;
    if (!SimguiSettings.RecentDocs.contains(s)) {
        SimguiSettings.RecentDocs.append(s);
    } else {
        SimguiSettings.RecentDocs.remove(s);
        SimguiSettings.RecentDocs.append(s);
    }
    if (SimguiSettings.RecentDocs.count()>8) {
        SimguiSettings.RecentDocs.removeFirst();
    }

    settings->setValue("RecentDocs",SimguiSettings.RecentDocs.join("*"));
    delete settings;
}

void SimguiApp::slotSaveDiagramToGraphicsFile()
{
    slotSaveSchematicToGraphicsFile(true);
}

void SimguiApp::slotSaveSchematicToGraphicsFile(bool diagram)
{
...
void SimguiApp::successExportMessages(bool ok)
{
    if (ok) {
