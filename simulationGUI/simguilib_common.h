...
#ifndef _SIMGUILIB_COMMON_H_
#define _SIMGUILIB_COMMON_H_
...
// simgui library files

enum LIB_PARSE_RESULT { SIMGUI_COMP_LIB_OK,
                        SIMGUI_COMP_LIB_IO_ERROR,
                        SIMGUI_COMP_LIB_CORRUPT,
                        SIMGUI_COMP_LIB_EMPTY };
...
inline int makeModelString (QString libname, QString compname, QString compstring, QString &modelstring)
{
    ...
    {
        return SIMGUI_COMP_LIB_CORRUPT;
    }
    ...
    for (int i = 0; i < symbolstringLines.count (); i++)
    {
        ...
        if(s == ".ID")
        {
            ...
            if (!getCompLineIntegers(symbolstringLines[i], &Text_x, &Text_y))  return SIMGUI_COMP_LIB_OK;
...
inline int parseComponentLibrary (QString filename, ComponentLibrary &library)
{
    ...
    {
        return SIMGUI_COMP_LIB_IO_ERROR;
    }
    ...
    Start = LibraryString.indexOf("<Simgui Library ");
    if(Start < 0)
    {
        return SIMGUI_COMP_LIB_CORRUPT;
    }
    ...
    while((Start=LibraryString.indexOf("\n<Component ", Start)) > 0)
    {
        ...
        if (library.defaultSymbol.isEmpty ())
        {
            ...
            if (result != SIMGUI_COMP_LIB_OK) return result;
        }
        else
        {
