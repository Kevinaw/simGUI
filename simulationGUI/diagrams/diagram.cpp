...
void Diagram::createAxisLabels()
{
  ...
  int   x, y, w, wmax = 0;
  QString Str;
  QFontMetrics  metrics(((Schematic*)SimguiMain->DocumentTab->currentPage())->font());   // get size of text
...
void Diagram::createSmithChart(Axis *Axis, int Mode)
{
  ...
  if(Axis->up > 1.0) {  // draw circle with |r|=1 ?
    ...
    if(Below)  y = 4;
    else  y = y2-4-SimguiSettings.font.pointSize();
...
void Diagram::createPolarDiagram(Axis *Axis, int Mode)
{
  ...
  int phi, tPos;
  int tHeight = SimguiSettings.font.pointSize() + 5;
