...
Schematic::Schematic(SimguiApp *App_, const QString& Name_)
    : SimguiDoc(App_, Name_)
{
  symbolMode = false;
  ...
  if(App) {
    ...
    viewport()->setPaletteBackgroundColor(SimguiSettings.BGColor);
...
void Schematic::setChanged(bool c, bool fillStack, char Op)
{
  ...
  if(symbolMode) {  // for symbol edit mode
    ...
    while(UndoSymbol.count() > SimguiSettings.maxUndo) { // "while..." because
  ...
  while(UndoStack.count() > SimguiSettings.maxUndo) { // "while..." because
...
{
  QFont Font = SimguiSettings.font;
...
// ---------------------------------------------------
// Saves this Simgui document. Returns the number of subcircuit ports.
int Schematic::save()
{
...
void Schematic::contentsDropEvent(QDropEvent *Event)
{
  if(dragIsOkay) {
    ...
    SimguiDoc *d = SimguiMain->getDoc(0);
