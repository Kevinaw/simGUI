 ...
 * \see SimguiApp::slotBuildModule() for the make output assignment.
 */
MessageDock::MessageDock(SimguiApp *App_): QWidget()
{
...
void MessageDock::slotCursor()
{
  ...
  // \todo set hightlight in SimguiDoc Verilog-A file?
  // move cursor? addt line number? highliht line number? set in focus
     ...
     * hightlings the line
     * SimguiApp::getDoc() //current should be a TextDoc
     */
//    SimguiDoc *foo =  SimguiMain->getDoc();
//    qWarning() << foo->DocName;
    ...
    if (gotoLine >= 0) {
        ...
        TextDoc * d = (TextDoc*)SimguiMain->getDoc();
