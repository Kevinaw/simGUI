...
void ComponentDialog::slotBrowseFile()
{
  QString s = QFileDialog::getOpenFileName (
          this,
          tr("Select a file"),
          SimguiSettings.SimguiWorkDir.path(),
          tr("All Files")+" (*.*);;"
  ...
  if(!s.isEmpty()) {
    ...
    QFileInfo file(s);
    if(SimguiSettings.SimguiWorkDir.exists(file.fileName()) &&
       SimguiSettings.SimguiWorkDir.absPath() == file.dirPath(true)) s = file.fileName();
    edit->setText(s);
  }
...
{
  Doc->App->editFile(SimguiSettings.SimguiWorkDir.filePath(edit->text()));
}
