...
MouseActions::MouseActions(SimguiApp* App_)
{
  ...
  ComponentMenu = new Q3PopupMenu(SimguiMain);
...
void MouseActions::MMoveWire2(Schematic *Doc, QMouseEvent *Event)
{
  ...
  SimguiMain->MouseDoubleClickAction = &MouseActions::MDoubleClickWire2;
  Doc->viewport()->update();
}
...
void MouseActions::MMoveMoving(Schematic *Doc, QMouseEvent *Event)
{
  ...
  MAx1 = MAx2;
  MAy1 = MAy2;
  SimguiMain->MouseMoveAction = &MouseActions::MMoveMoving2;
  SimguiMain->MouseReleaseAction = &MouseActions::MReleaseMoving;
...
void MouseActions::MMovePaste(Schematic *Doc, QMouseEvent *Event)
{
  ...
  drawn = true;
  SimguiMain->MouseMoveAction = &MouseActions::MMoveMoving2;
  SimguiMain->MouseReleaseAction = &MouseActions::MReleasePaste;
}
...
// FIXME #warning                      SimguiSettings.BGColor);
...
void MouseActions::rightPressMenu(Schematic *Doc, QMouseEvent *Event, float fX, float fY)
{
  ...
  while(true) {
    if(focusElement) {
      ...
      ComponentMenu->insertItem(
         QObject::tr("Edit Properties"), SimguiMain, SLOT(slotEditElement()));

      if((focusElement->Type & isComponent) == 0) break;
    }
    else {
      SimguiMain->symEdit->addTo(ComponentMenu);
      SimguiMain->fileSettings->addTo(ComponentMenu);
    }
    if(!SimguiMain->moveText->isOn())
      SimguiMain->moveText->addTo(ComponentMenu);
    break;
  }
  while(true) {
    if(focusElement)
      if(focusElement->Type == isGraph) break;
    if(!SimguiMain->onGrid->isOn())
      SimguiMain->onGrid->addTo(ComponentMenu);
    SimguiMain->editCopy->addTo(ComponentMenu);
    if(!SimguiMain->editPaste->isOn())
      SimguiMain->editPaste->addTo(ComponentMenu);
    break;
  }
  ...
  while (true) {
      if (focusElement) {
          if (focusElement->Type == isDiagram) {
              ...
              ComponentMenu->insertItem(QObject::tr("Export as image"), SimguiMain,
  ...
  if(!SimguiMain->editDelete->isOn())
    SimguiMain->editDelete->addTo(ComponentMenu);
  if(focusElement) if(focusElement->Type == isMarker) {
    ...
    ComponentMenu->insertItem(s, SimguiMain, SLOT(slotPowerMatching()));
    if( ((Marker*)focusElement)->pGraph->Var.left(2) == "S[" )
      ComponentMenu->insertItem(QObject::tr("2-port matching"), SimguiMain,
  ...
  do {
    if(focusElement) {
      ...
      if(focusElement->Type == isGraph) {
        SimguiMain->graph2csv->addTo(ComponentMenu);
        break;
      }
    }
    ComponentMenu->insertSeparator();
    if(focusElement) if(focusElement->Type & isComponent)
      if(!SimguiMain->editActivate->isOn())
        SimguiMain->editActivate->addTo(ComponentMenu);
    if(!SimguiMain->editRotate->isOn())
      SimguiMain->editRotate->addTo(ComponentMenu);
    if(!SimguiMain->editMirror->isOn())
      SimguiMain->editMirror->addTo(ComponentMenu);
    if(!SimguiMain->editMirrorY->isOn())
      SimguiMain->editMirrorY->addTo(ComponentMenu);

    // right-click menu to go into hierarchy
    if(focusElement) {
      if(focusElement->Type & isComponent)
	if(((Component*)focusElement)->Model == "Sub")
	  if(!SimguiMain->intoH->isOn())
	    SimguiMain->intoH->addTo(ComponentMenu);
    }
    // right-click menu to pop out of hierarchy
    if(!focusElement)
      if(!SimguiMain->popH->isOn())
	SimguiMain->popH->addTo(ComponentMenu);
  } while(false);
...
void MouseActions::MPressSelect(Schematic *Doc, QMouseEvent *Event, float fX, float fY)
{
  ...
  switch(focusElement->Type)
  {
    case isPaintingResize:  // resize painting ?
      ...
      SimguiMain->MouseReleaseAction = &MouseActions::MReleaseResizePainting;
      SimguiMain->MouseMoveAction = &MouseActions::MMoveResizePainting;
      SimguiMain->MousePressAction = 0;
      SimguiMain->MouseDoubleClickAction = 0;
    ...
    case isDiagramResize:  // resize diagram ?
      ...
      SimguiMain->MouseReleaseAction = &MouseActions::MReleaseResizeDiagram;
      SimguiMain->MouseMoveAction = &MouseActions::MMoveSelect;
      SimguiMain->MousePressAction = 0;
      SimguiMain->MouseDoubleClickAction = 0;
      Doc->grabKeyboard(); // no keyboard inputs during move actions
      // Update matching wire label highlighting
    ...
    case isDiagramVScroll:
      ...
      switch(No)
      {
        ...
        case 2:  // move scroll bar with mouse cursor
          SimguiMain->MouseMoveAction = &MouseActions::MMoveScrollBar;
          SimguiMain->MousePressAction = 0;
          SimguiMain->MouseDoubleClickAction = 0;
          Doc->grabKeyboard();  // no keyboard inputs during move actions
    ...
    case isComponentText:  // property text of component ?
      ...
      MAx3 = No;
      SimguiMain->slotApplyCompText();
      // Update matching wire label highlighting
      Doc->highlightWireLabels ();
      return;

    case isNode:
      if (SimguiSettings.NodeWiring)
      {
        ...
        SimguiMain->MouseMoveAction = &MouseActions::MMoveWire2;
        SimguiMain->MousePressAction = &MouseActions::MPressWire2;
        SimguiMain->MouseReleaseAction = 0; // if function is called from elsewhere
        SimguiMain->MouseDoubleClickAction = 0;

        formerAction = SimguiMain->select; // to restore action afterwards
        SimguiMain->activeAction = SimguiMain->insWire;

        SimguiMain->select->blockSignals(true);
        SimguiMain->select->setOn(false);
        SimguiMain->select->blockSignals(false);

        SimguiMain->insWire->blockSignals(true);
        SimguiMain->insWire->setOn(true);
        SimguiMain->insWire->blockSignals(false);
        // Update matching wire label highlighting
        Doc->highlightWireLabels ();
        return;
      }
  ...
  if(focusElement == 0) {
    ...
    MAy2 = 0;
    SimguiMain->MouseReleaseAction = &MouseActions::MReleaseSelect2;
    SimguiMain->MouseMoveAction = &MouseActions::MMoveSelect;
  }
  else
  {
    ...
    Doc->setOnGrid(MAx1, MAy1);
    SimguiMain->MouseMoveAction = &MouseActions::MMoveMoving;
  }
  // Update matching wire label highlighting
  Doc->highlightWireLabels ();
}
...
//    if(Event->button() != Qt::LeftButton) return;
    ...
    MAy2 = 0;
    SimguiMain->MousePressAction = 0;
    SimguiMain->MouseReleaseAction = &MouseActions::MReleaseActivate;
    SimguiMain->MouseMoveAction = &MouseActions::MMoveSelect;
  }
  Doc->viewport()->update();
  drawn = false;
...
void MouseActions::MPressWire2(Schematic *Doc, QMouseEvent *Event, float fX, float fY)
{
  ...
  case Qt::LeftButton :
    ...
    if(set1 & 2) {
      ...
      if(formerAction) {
        // ...restore old action
        SimguiMain->select->setOn(true);
      }
      else {
        // ...start a new wire
        SimguiMain->MouseMoveAction = &MouseActions::MMoveWire1;
        SimguiMain->MousePressAction = &MouseActions::MPressWire1;
        SimguiMain->MouseDoubleClickAction = 0;
      }
    }
...
void MouseActions::MPressMoveText(Schematic *Doc, QMouseEvent*, float fX, float fY)
{
  ...
  if(focusElement) {
    ...
    Doc->viewport()->update();
    drawn = false;
    SimguiMain->MouseMoveAction = &MouseActions::MMoveMoveText;
    SimguiMain->MouseReleaseAction = &MouseActions::MReleaseMoveText;
    Doc->grabKeyboard();  // no keyboard inputs during move actions
  }
}
...
void MouseActions::MPressZoomIn(Schematic *Doc, QMouseEvent*, float fX, float fY)
{
  ...
  SimguiMain->MouseMoveAction = &MouseActions::MMoveSelect;
  SimguiMain->MouseReleaseAction = &MouseActions::MReleaseZoomIn;
  Doc->grabKeyboard();  // no keyboard inputs during move actions
  Doc->viewport()->update();
  drawn = false;
}
...
void MouseActions::MReleaseSelect(Schematic *Doc, QMouseEvent *Event)
{
  ...
  SimguiMain->MousePressAction = &MouseActions::MPressSelect;
  SimguiMain->MouseReleaseAction = &MouseActions::MReleaseSelect;
  SimguiMain->MouseDoubleClickAction = &MouseActions::MDoubleClickSelect;
  SimguiMain->MouseMoveAction = 0;   // no element moving
  Doc->highlightWireLabels ();
  Doc->viewport()->update();
  drawn = false;
}
...
void MouseActions::MReleaseActivate(Schematic *Doc, QMouseEvent *Event)
{
  ...
  SimguiMain->MouseMoveAction = &MouseActions::MMoveActivate;
  SimguiMain->MousePressAction = &MouseActions::MPressActivate;
  SimguiMain->MouseReleaseAction = 0;
  SimguiMain->MouseDoubleClickAction = 0;
  Doc->highlightWireLabels ();
  Doc->viewport()->update();
  drawn = false;
...
void MouseActions::MReleasePaste(Schematic *Doc, QMouseEvent *Event)
{
  ...
  case Qt::LeftButton :
    ...
    SimguiMain->MouseMoveAction = &MouseActions::MMovePaste;
    SimguiMain->MousePressAction = 0;
    SimguiMain->MouseReleaseAction = 0;
    SimguiMain->MouseDoubleClickAction = 0;
...
void MouseActions::MReleaseMoveText(Schematic *Doc, QMouseEvent *Event)
{
  ...
  SimguiMain->MouseMoveAction = &MouseActions::MMoveMoveTextB;
  SimguiMain->MouseReleaseAction = 0;
  Doc->releaseKeyboard();  // allow keyboard inputs again
...
void MouseActions::MReleaseZoomIn(Schematic *Doc, QMouseEvent *Event)
{
  ...
  SimguiMain->MouseMoveAction = &MouseActions::MMoveZoomIn;
  SimguiMain->MouseReleaseAction = 0;
  Doc->releaseKeyboard();  // allow keyboard inputs again
}
...
{
  Doc->releaseKeyboard();  // allow keyboard inputs again
  SimguiMain->editText->setHidden(true);
...
void MouseActions::MDoubleClickWire2(Schematic *Doc, QMouseEvent *Event)
{
  ...
  if(formerAction)
    SimguiMain->select->setOn(true);  // restore old action
  else {
    SimguiMain->MouseMoveAction = &MouseActions::MMoveWire1;
    SimguiMain->MousePressAction = &MouseActions::MPressWire1;
    SimguiMain->MouseDoubleClickAction = 0;
