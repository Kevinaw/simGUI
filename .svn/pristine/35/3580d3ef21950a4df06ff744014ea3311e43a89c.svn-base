...
SpiceDialog::SpiceDialog(SimguiApp* App_, SpiceFile *c, Schematic *d)
...
void SpiceDialog::slotButtBrowse()
{
    ...
    if(SimguiSettings.SimguiWorkDir.exists(Info.fileName()) &&
            SimguiSettings.SimguiWorkDir.absPath() == Info.dirPath(true)) s = Info.fileName();
    FileEdit->setText(s);
...
bool SpiceDialog::loadSpiceNetList(const QString& s)
{
    ...
    QFileInfo FileInfo(SimguiSettings.SimguiWorkDir, s);
    ...
    if (preprocessor != "none")
    {
        ...
        script = SimguiSettings.BinDir + script;
        ...
        QFile PrepFile;
        QFileInfo PrepInfo(SimguiSettings.SimguiWorkDir, s + ".pre");
        ...
        FileInfo = QFileInfo(SimguiSettings.SimguiWorkDir, s + ".pre");
    }

    // Now do the spice->simgui netlist conversion using the simguiconv program ...
    SimguiConv = new QProcess(this);
    ...
    connect(SimguiConv, SIGNAL(readyReadStandardOutput()), SLOT(slotGetNetlist()));
    connect(SimguiConv, SIGNAL(readyReadStandardError()), SLOT(slotGetError()));
    ...
    connect(SimguiConv, SIGNAL(finished(int, QProcess::ExitStatus)), MBox, SLOT(close()));

    SimguiConv->start(Program, Arguments);

//    if((SimguiConv->state() != QProcess::Starting) && (SimguiConv->state()!=QProcess::Running))
    if(!SimguiConv->Running)
    {
        QMessageBox::critical(this, tr("Error"),
                              tr("Cannot execute \"%1\".").arg(SimguiSettings.BinDir + "simguiconv" + executableSuffix));
        return false;
    }
    ...
    if(!Error.isEmpty())
        QMessageBox::critical(this, tr("SimguiConv Error"), Error);
...
{
    Error += QString(SimguiConv->readAllStandardError ());
}
...
void SpiceDialog::slotGetNetlist()
{
    ...
    int i;
    QString s;
    Line += QString(SimguiConv->readAllStandardOutput ());
...
{
    Doc->App->editFile(SimguiSettings.SimguiWorkDir.filePath(FileEdit->text()));
}
