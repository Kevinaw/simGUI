...
int Component::getTextSelected(int x_, int y_, float Corr)
{
  ...
  QFontMetrics  metrics(SimguiSettings.font);
  if(showName) {
...
void Component::paintScheme(Schematic *p)
{
  ...
  if(Model.at(0) == '.') {   // is simulation component (dc, ac, ...)
    ...
    float Scale =
          ((Schematic*)SimguiMain->DocumentTab->currentPage())->Scale;
    newFont.setPointSizeFloat(float(Scale) * SimguiSettings.largeFontSize);
    newFont.setWeight(QFont::DemiBold);
