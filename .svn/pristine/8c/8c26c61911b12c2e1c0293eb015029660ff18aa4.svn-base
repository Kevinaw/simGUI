...
{
  QFontMetrics  metrics(SimguiSettings.font);   // get size of text
  ...
  else
  {
    ...
    if (FileInfo.fileName () == FileName)
    {
        ...
        if (containingSchematic)
        {
            ...
            for (int i = 0; i < SimguiMain->spiceExtensions.count (); i++)
            {
                QFileInfo localFIleInfo (schematicFileInfo.canonicalPath ()
                                         + "/" + baseName + SimguiMain->spiceExtensions[i]);
    ...
    QMutex mutex;
    mutex.lock();
    QString hashsearchresult = SimguiMain->spiceNameHash.value(baseName);
    mutex.unlock();
...
bool SpiceFile::recreateSubNetlist(QString *SpiceFile, QString *FileName)
{
  ...
  QProcessEnvironment env = QProcessEnvironment::systemEnvironment();
  env.insert("PATH", env.value("PATH") );
  SimguiConv->setProcessEnvironment(env);

  qDebug() << "Command:" << prog << com.join(" ");
//  SimguiConv->start(com.join(" "));
  SimguiConv->start(prog, com);
  ...
  connect(SimguiConv, SIGNAL(finished(int,QProcess::ExitStatus)), SLOT(slotExited()));

  if(SimguiConv->state()!=QProcess::Running&&
          SimguiConv->state()!=QProcess::Starting) {
    ErrText += QObject::tr("COMP ERROR: Cannot start SimguiConv!");
    return false;
  }
  (*outstream) << NetText;
  (*filstream) << NetText;
  ...
  QMessageBox *MBox = new QMessageBox(QObject::tr("Info"),
	       QObject::tr("Converting SPICE file \"%1\".").arg(*SpiceFile),
               ...
               QMessageBox::NoButton, QMessageBox::NoButton, 0, 0, true,
	       Qt::WStyle_DialogBorder | Qt::WDestructiveClose);
  connect(SimguiConv, SIGNAL(finished(int)), MBox, SLOT(close()));
  MBox->exec();
...
{
  ErrText += QString(SimguiConv->readAllStandardError());
}
...
void SpiceFile::slotGetNetlist()
{
  int i;
  QString s;
  NetLine += QString(SimguiConv->readAllStandardOutput());
...
{
  if (SimguiConv->exitStatus() != QProcess::NormalExit) {
    NetText = "";
  }
  else {
    if(makeSubcircuit) {
