 ...
 */
TextDoc::TextDoc(SimguiApp *App_, const QString& Name_) : SimguiDoc(App_, Name_), QPlainTextEdit()
{
  TextFont = QFont("Courier New");
  TextFont.setPointSize(SimguiSettings.font.pointSize()-1);
  ...
  //TODO (not supported) setUndoDepth(SimguiSettings.maxUndo);
  setLanguage (Name_);
  ...
  if(App) {
    ...
    setPaletteBackgroundColor(SimguiSettings.BGColor);
...
bool TextDoc::saveSettings (void)
{
  ...
  stream << "Textfile settings file, Simgui " PACKAGE_VERSION "\n"
 ...
 * See also SimguiApp::slotEditActivate
 */
