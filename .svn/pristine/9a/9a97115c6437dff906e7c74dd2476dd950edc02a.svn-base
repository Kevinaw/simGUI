...
// -------------------------------------------------------------
// Creates a Simgui file format (without document properties) in the returning
...
int Schematic::saveDocument()
{
  ...
  stream << "<Simgui Schematic " << PACKAGE_VERSION << ">\n";
...
//      QDir prefix = QDir(SimguiSettings.BinDir);
      ...
      qDebug() << "homedir"  << SimguiSettings.SimguiHomeDir.absolutePath();

      vaFile = SimguiSettings.SimguiWorkDir.filePath(fileBase()+".va");

      QStringList Arguments;
      Arguments << QDir::toNativeSeparators(vaFile)
                << "-I" << QDir::toNativeSeparators(include.absolutePath())
                << "-e" << QDir::toNativeSeparators(include.absFilePath("simguiMODULEguiJSONsymbol.xml"))
                << "-A" << "dyload";
...
//      env.insert("PATH", env.value("PATH") );
      ...
      QFile f1(SimguiSettings.SimguiWorkDir.filePath(fileBase()+"_props.json"));
      QFile f2(SimguiSettings.SimguiWorkDir.filePath(fileBase()+"_sym.json"));
      ...
      QFile f3(SimguiSettings.SimguiWorkDir.filePath(fileBase()+"_symbol.json"));
...
//    else if(cstr == "EArc") p = new EllipseArc();
    if(cstr == ".PortSym") p = new PortSymbol();
...
bool Schematic::loadDocument()
{
  ...
  while(!stream.atEnd()) {
    ...
    if(Line == "<Symbol>") {
      if ((Schematic*)SimguiMain != 0) // GUI running, load Symbols
    ...
    if ((Schematic*)SimguiMain != 0) { // GUI running, load Diagrams, Paintings
    if(Line == "<Diagrams>") {
                              ...
                              QTextEdit *ErrText)
{
  ...
  if((allTypes & isAnalogComponent) == 0) {
    if(allTypes == 0) {
      ...
      // be a simulation within a SPICE file. Otherwise Simguiator will
      // output an error.
      isAnalog = true;
  ...
  if(allTypes & isAnalogComponent)
    ...
    stream << "--";
  stream << " Simgui " << PACKAGE_VERSION << "  " << DocName << "\n";
