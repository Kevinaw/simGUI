			...
			: QDialog(parent_) //, 0, TRUE, Qt::WDestructiveClose)
{
  ...
  if(create_) {  // create or extract package ?
    ...
    // ...........................................................
    // insert all projects
    QStringList PrDirs = SimguiSettings.SimguiHomeDir.entryList("*", QDir::Dirs, QDir::Name);
...
{
  QFile File;
  QDir myDir(SimguiSettings.SimguiHomeDir.absPath() + QDir::separator() + "user_lib");
  QStringList Entries = myDir.entryList("*", QDir::Files, QDir::Name);
  QStringList::iterator it;
  for(it = Entries.begin(); it != Entries.end(); ++it) {
    File.setName(myDir.absFilePath(*it));
...
void PackageDialog::slotCreate()
{
  ...
  strcpy(Header, "Simgui package " PACKAGE_VERSION);
  ...
  while(i.hasNext()) {
    ...
    if(p->isChecked()) {
      ...
      s = SimguiSettings.SimguiHomeDir.absPath() + QDir::separator() + s;
  ...
  QMessageBox::information(this, tr("Info"),
          tr("Successfully created Simgui package!"));
  accept();
}
...
void PackageDialog::extractPackage()
{
  ...
  if(!PkgFile.open(QIODevice::ReadOnly)) {
    if(Info.extension().isEmpty()) s += ".simgui";
    PkgFile.setName(s);
    if(!PkgFile.open(QIODevice::ReadOnly)) {
  ...
  QDir currDir = SimguiSettings.SimguiHomeDir;
  QString Version;
  Q_UINT16 Checksum;
  ...
  QByteArray Content = PkgFile.readAll();
  if(strncmp(Content.data(), "Simgui package ", 13) != 0) {
...
int PackageDialog::extractLibrary(QFile& PkgFile, Q_UINT32 Count)
{
  ...
  p = Content.data();
  QFile File(SimguiSettings.SimguiHomeDir.absPath() +
