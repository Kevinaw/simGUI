...
class SimguiApp : public QMainWindow {
  Q_OBJECT
public:
  SimguiApp();
 ~SimguiApp();
  QSettings *simguiSettings;
  bool closeAllFiles();
  static int testFile(const QString&);
  bool gotoPage(const QString&);   // to load a document
  SimguiDoc *getDoc(int No=-1);
  SimguiDoc* findDoc (QString, int * Pos = 0);
  QString fileType (const QString&);

  QString ProjName;   // name of the project, that is open
  //QList<QString> simguiPathList; // the simgui path list for subcircuits and spice files
